...
<div class="modal fade" id="addDistributionModal" tabindex="-1" role="dialog">
...
<div class="col-md-6">
    <div class="form-group">
        <label>المدرسة المستلمة <span class="text-danger">*</span></label>
        <select name="school_id" class="form-control select2" style="width: 100%;" required>
            <option value="">اختر الإدارة أولاً</option>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" name="order_date" class="form-control" value="<?php echo e(date('Y-m-d')); ?>" required>
                            </div>
                        </div>
                    </div>
                                ...
                                <tr class="item-row">
                                    <td>
                                        <select name="items[0][product_id]" class="form-control product-select" required>
                                            <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($product->id); ?>" <?php echo e($loop->first ? 'selected' : ''); ?>>
                                                    <?php echo e($product->name); ?> (<?php echo e(number_format($product->price)); ?> ج.م)
                                                </option>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </select>
                                    </td>
                                    <td>
                                        <input type="number" name="items[0][quantity]"
                                            class="form-control text-center quantity-input" placeholder="0" min="1"
                                            step="1" required>
                                    </td>
                                    <td>
...
$(document).ready(function() {
    ...
    $('#addItem').click(function() {
            ...
            <tr class="item-row">
                <td>
                    <select name="items[${itemIndex}][product_id]" class="form-control product-select" required>
                        <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($product->id); ?>"><?php echo e($product->name); ?> (<?php echo e(number_format($product->price)); ?> ج.م)</option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </td>
                <td>
        ...
        $('.product-select').select2({
            dropdownParent: $('#addDistributionModal')
        });
    });
...
$('#addDistributionForm').on('submit', function(e) {
    ...
    $.ajax({
        url: "<?php echo e(route('admin.distribution_orders.store')); ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
</script><?php /**PATH C:\xampp\htdocs\updating lum\resources\views/backend/distributions/partials/modals/add.blade.php ENDPATH**/ ?>